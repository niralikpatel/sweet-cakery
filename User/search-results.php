...
<body>
    ...
    <div class="row">
        ...
        if (!empty($category_id)) {
            // Fetch the total number of items in the selected category
            $total_items_query = $conn->prepare("SELECT COUNT(*) FROM items WHERE category_id = :category_id");
            $total_items_query->bindParam(':category_id', $category_id, PDO::PARAM_INT);
        } else {
            // Fetch the total number of items if no category is selected
        ...
        if (!empty($category_id)) {
            // Fetch items based on category with pagination
            $sql = "SELECT * FROM items WHERE category_id = :category_id LIMIT :limit OFFSET :offset";
            $query = $conn->prepare($sql);
            $query->bindParam(':category_id', $category_id, PDO::PARAM_INT);
        } else {
            // Fetch all items with pagination if no category is selected
            ...
            foreach ($results as $result) { ?>
                <div class="col-lg-3 col-md-4 col-sm-6 mix">
                    <div class="card">
                        ...
                        <div class="price-tag">$<?php echo htmlentities($result->price); ?></div>
                        <div class="card-body">
    ...
    <div class="pagination">
        <?php if ($page > 1) { ?>
            <a href="?category_id=<?php echo htmlentities($category_id); ?>&page=<?php echo $page - 1; ?>">&laquo; Previous</a>
        <?php } ?>

        <?php for ($i = 1; $i <= $total_pages; $i++) { ?>
            <a href="?category_id=<?php echo htmlentities($category_id); ?>&page=<?php echo $i; ?>" <?php if ($i == $page) echo 'class="active"'; ?>><?php echo $i; ?></a>
        <?php } ?>

        <?php if ($page < $total_pages) { ?>
            <a href="?category_id=<?php echo htmlentities($category_id); ?>&page=<?php echo $page + 1; ?>">Next &raquo;</a>
        <?php } ?>
    </div>
